...
* Add `HeartbeatInterval` option to the scheduler (PR: https://github.com/adaptyteam/fox-asynq/pull/956)
* Add `RedisUniversalClient` support to periodic task manager (PR: https://github.com/adaptyteam/fox-asynq/pull/958)
* Add `--insecure` flag to CLI dash command (PR: https://github.com/adaptyteam/fox-asynq/pull/980)
* Add logging for registration errors (PR: https://github.com/adaptyteam/fox-asynq/pull/657)

### Fixes
- Perf: Use string concat inplace of fmt.Sprintf in hotpath (PR: https://github.com/adaptyteam/fox-asynq/pull/962)
- Perf: Init map with size (PR: https://github.com/adaptyteam/fox-asynq/pull/673)
- Fix: `Scheduler` and `PeriodicTaskManager` graceful shutdown (PR: https://github.com/adaptyteam/fox-asynq/pull/977)
- Fix: `Server` graceful shutdown on UNIX systems (PR: https://github.com/adaptyteam/fox-asynq/pull/982)

## [0.25.0] - 2024-10-29

### Upgrades
- Minumum go version is set to 1.22 (PR: https://github.com/adaptyteam/fox-asynq/pull/925)
- Internal protobuf package is upgraded to address security advisories (PR: https://github.com/adaptyteam/fox-asynq/pull/925)
- Most packages are upgraded
- CI/CD spec upgraded

### Added
- `IsPanicError` function is introduced to support catching of panic errors when processing tasks (PR: https://github.com/adaptyteam/fox-asynq/pull/491)
- `JanitorInterval` and `JanitorBatchSize` are added as Server options (PR: https://github.com/adaptyteam/fox-asynq/pull/715)
- `NewClientFromRedisClient` is introduced to allow reusing an existing redis client (PR: https://github.com/adaptyteam/fox-asynq/pull/742)
- `TaskCheckInterval` config option is added to specify the interval between checks for new tasks to process when all queues are empty (PR: https://github.com/adaptyteam/fox-asynq/pull/694)
- `Ping` method is added to Client, Server and Scheduler ((PR: https://github.com/adaptyteam/fox-asynq/pull/585))
- `RevokeTask` error type is introduced to prevent a task from being retried or archived (PR: https://github.com/adaptyteam/fox-asynq/pull/882)
- `SentinelUsername` is added as a redis config option (PR: https://github.com/adaptyteam/fox-asynq/pull/924)
- Some jitter is introduced to improve latency when fetching jobs in the processor (PR: https://github.com/adaptyteam/fox-asynq/pull/868)
- Add task enqueue command to the CLI (PR: https://github.com/adaptyteam/fox-asynq/pull/918)
- Add a map cache (concurrent safe) to keep track of queues that ultimately reduces redis load when enqueuing tasks (PR: https://github.com/adaptyteam/fox-asynq/pull/946)

### Fixes
- Archived tasks that are trimmed should now be deleted (PR: https://github.com/adaptyteam/fox-asynq/pull/743)
- Fix lua script when listing task messages with an expired lease (PR: https://github.com/adaptyteam/fox-asynq/pull/709)
- Fix potential context leaks due to cancellation not being called (PR: https://github.com/adaptyteam/fox-asynq/pull/926)
...
### Added
- `PreEnqueueFunc`, `PostEnqueueFunc` is added in `Scheduler` and deprecated `EnqueueErrorHandler` (PR: https://github.com/adaptyteam/fox-asynq/pull/476)
...
- Fixed overflow issue with 32-bit systems (For details, see https://github.com/adaptyteam/fox-asynq/pull/426)
...
- Introduced new configuration for `DelayedTaskCheckInterval`. See [godoc](https://godoc.org/github.com/adaptyteam/fox-asynq) for more details.
...
- Fixed latency issue around memory usage (see https://github.com/adaptyteam/fox-asynq/issues/309).
...
- Free unique lock when task is deleted (https://github.com/adaptyteam/fox-asynq/issues/275).
...
**IMPORTATNT**: All `Inspector` related code are moved to subpackage "github.com/adaptyteam/fox-asynq/inspeq"
...
- `Scheduler` type is added to enable periodic tasks. See the godoc for its APIs and [wiki](https://github.com/adaptyteam/fox-asynq/wiki/Periodic-Tasks) for the getting-started guide.
...
**IMPORTANT**: If you are upgrading from a previous version, please install the latest version of the CLI `go get -u github.com/adaptyteam/fox-asynq/tools/asynq` and run `asynq migrate` command. No process should be writing to Redis while you run the migration command.
...
- Fixes issue of same tasks processed by more than one worker (https://github.com/adaptyteam/fox-asynq/issues/90).

## [0.9.3] - 2020-06-12

### Fixed

- Fixes the JSON number overflow issue (https://github.com/adaptyteam/fox-asynq/issues/166).
...
- [Fixed cancelfunc leak](https://github.com/adaptyteam/fox-asynq/pull/145)
